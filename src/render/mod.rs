...
pub fn render(gitlab_project_pipelines: &GitlabProjectPipelines) -> Vec<String> {
    ...
    for (pip, jobs) in &gitlab_project_pipelines.pipelines {
        overview.append(&mut pipeline::generate_pipeline_overview(
            pip,
            width as usize,
        ));
        overview.append(&mut generate_job_overview(&jobs, width as usize));
    }
    overview
}
